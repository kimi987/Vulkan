...
namespace vkInit {
	/*
		...
		capablities: no. of images and supported sizes
	...
	*/
	...
	SwapChainSupportDetails query_swapchain_support(vk::PhysicalDevice device, vk::SurfaceKHR surface, bool debug) {
		...
		if (debug)
		{
			...
			std::cout << "\t mininum image count: " << support.capabilities.minImageCount << std::endl;
			std::cout << "\t maxinum image count: " << support.capabilities.maxImageCount << std::endl;
	...
	*/
	vk::Extent2D choose_swapchain_extent(uint32_t width, uint32_t height, vk::SurfaceCapabilitiesKHR capabilites) {
		if (capabilites.currentExtent.width != UINT32_MAX)
		{
			...
			extent.width = std::min(capabilites.maxImageExtent.width, std::max(capabilites.minImageExtent.width, extent.width));

			extent.height = std::min(capabilites.maxImageExtent.height, std::max(capabilites.minImageExtent.height, extent.height));
	...
	*/
	SwapChainBundle create_swapchain(vk::Device logicalDevice, vk::PhysicalDevice physicalDevice, vk::SurfaceKHR surface, int width, int height, bool debug) {
		...
		vk::PresentModeKHR mode = choose_swapchain_present_mode(support.presentModes);

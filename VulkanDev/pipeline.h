	...
	*/
	...
	GraphicsPipelineOutBundle create_graphics_pipeline(GraphicsPipelineInBundle specification, bool debug) {
		...
		*/
		...
		vk::ShaderModule vertexShader = vkUtils::createModule(specification.vertexFilePath, specification.device, debug);
		...
		rasterizer.depthBiasEnable = VK_FALSE; //Depth bias can be useful in shadow map
		...
		vk::ShaderModule fragmentShader = vkUtils::createModule(specification.fragmentFilePath, specification.device, debug);
